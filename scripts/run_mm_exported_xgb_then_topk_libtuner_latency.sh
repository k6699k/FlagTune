...
show_help() {
  cat <<'EOF'
Usage:
  ./run_mm_exported_xgb_then_topk_libtuner_latency.sh --export-model-dir /path/to/exported_xgb_model [args]
...
This script skips XGBoost training. It runs:
  1. mm_xgboost_predict_shape_configs.py from an exported model directory
  2. run_mm_topk_configs_libtuner_latency.py for each generated top-k YAML
  3. mm_speedup_plot.py for each model output directory
