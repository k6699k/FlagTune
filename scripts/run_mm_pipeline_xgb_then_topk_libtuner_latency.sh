...
show_help() {
  cat <<'EOF'
Usage:
  ./run_mm_pipeline_xgb_then_topk_libtuner_latency.sh [args]
bash run_mm_pipeline_xgb_then_topk_libtuner_latency.sh \
  ...
  --debug-libtuner \
  --show-child-output
The script runs the new FlagTune.mm_pipeline module entry points in order:
  1. FlagTune.mm_pipeline.cli.train_xgb --export-only
  2. FlagTune.mm_pipeline.cli.predict_topk, one top-k YAML per --predict-model
  3. FlagTune.mm_pipeline.cli.run_topk_latency for each generated top-k YAML
...
Most arguments are auto-routed by their original Python option names:
  ./run_mm_pipeline_xgb_then_topk_libtuner_latency.sh --db /path/model_TunedConfig.db --train-mode shape50_config100 --expand-count 480 --top-k 10 --warmup 1000 --rep 100 --parallel 8 --gpus 0,1,2,3,4,5,6,7

If an option name is ambiguous, or you want to be explicit, split args by target:
  ./run_mm_pipeline_xgb_then_topk_libtuner_latency.sh --xgb --seed 2026 --n-jobs 8 --run --seed 7 --parallel 8
...
FlagTune.mm_pipeline.cli.run_topk_latency examples:
...
xgb_cmd=(
  "$PYTHON_BIN" -m FlagTune.mm_pipeline.cli.train_xgb
  --db "$TRAIN_DB_FILE"
...
run_timed "FlagTune.mm_pipeline.cli.train_xgb export model" env "${CPU_ONLY_ENV[@]}" "${xgb_cmd[@]}"
